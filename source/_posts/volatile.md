...
***数据传递和存储速率：***
&nbsp;&nbsp;硬盘<内存<cpu
&nbsp;&nbsp;MySQL装在硬盘上，Redis装在内存，cpu只管计算不管存储，因为cpu的计算速度很快，所以在内存和cpu之间还存在着缓存cache
&nbsp;&nbsp;我们平时说的内存条硬件就是主内存
...
这就可能存在一个线程A修改了共享变量X的值但还未写回主内存中时，另一个线程B又对主内存中的同一个共享变量X进行操作，但此时A线程工作内存中共享变量X对线程B来说是不可见的，这种工作内存与主内存同步延迟现象就造成了可见性问题。

### 2.2 VolatileDemo代码演示可见性
```java
...
### 2.3 原子性 <font color=#FF000>（number++在多线程下是非线程安全的）</font>
原子性是指不可分割，完整性，即某个线程正在做某个业务时，中间不可以被加塞或者被分割。需要整体完整，要么同时成功，要么同时失败。

### 2.4 VolatileDemo不保证原子性代码演示
```java
import java.util.concurrent.TimeUnit;
